...
/**
 * עוזר AI גנרי — מוגבל לשיטה בלבד, לא נותן תשובות תוכן.
 * המסלול /api/runner-assistant טוען את ה-system prompt מתוך tasks.definition
 * לפי taskId, כך שאותו רכיב/endpoint משרתים כל משימה עתידית בלי קוד חדש.
 */
export function Assistant({ taskId }: { taskId: string }) {
  ...
  const { messages, sendMessage, status, error } = useChat({
    transport: new DefaultChatTransport({
      api: `/api/runner-assistant?taskId=${encodeURIComponent(taskId)}`,
    }),
  });
